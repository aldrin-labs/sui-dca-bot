module cetus_clmm::clmm_math {
    ...
    public fun get_delta_up_from_input(
        ...
        let sqrt_price_diff = if (current_sqrt_price > targ_et_sqrt_price) {
            current_sqrt_price - targ_et_sqrt_price
        } else {
            targ_et_sqrt_price - current_sqrt_price
        };
        if (sqrt_price_diff == 0 || liquidity == 0) {
            return 0
        };
        if (a_to_b) {
            ...
            //     abort EMULTIPLICATION_OVERFLOW
            // };
            // let denomminator = full_math_u128::full_mul(current_sqrt_price, targ_et_sqrt_price);
            ...
            let denominator = full_math_u128::full_mul(current_sqrt_price, targ_et_sqrt_price);
    ...
    public fun compute_swap_step(
        ...
        let next_sqrt_price = targ_et_sqrt_price;
        let amount_in: u64 = 0;
        ...
        if (a2b) {
            assert!(current_sqrt_price >= targ_et_sqrt_price, EINVALID_SQRT_PRICE_INPUT)
        } else {
            assert!(current_sqrt_price < targ_et_sqrt_price, EINVALID_SQRT_PRICE_INPUT)
        };
        //let a_to_b = current_sqrt_price >= targ_et_sqrt_price;

        if (by_amount_in) {
            let amount_remain =
                full_math_u64::mul_div_floor(amount, (FEE_RATE_DENOMINATOR - fee_rate), FEE_RATE_DENOMINATOR);
            let max_amount_in =
                get_delta_up_from_input(current_sqrt_price, targ_et_sqrt_price, liquidity, a2b);
            ...
            if (max_amount_in > (amount_remain as u256)) {
                ...
                next_sqrt_price = targ_et_sqrt_price;
            };
    ...
    fun test_compute_swap_step() {
        let (current_sqrt_price, targ_et_sqrt_price, liquidity, amount, fee_rate) = (
            1u128 << 64,
            2u128 << 64,
            1000u128 << 32,
            20000,
